...
export const applyTidbProxySuppressions = (stack: cdk.Stack): void => {
  acknowledgeRules(stack, [
    ...
    },
    {
      ...
      reason:
        'ECS UpdateTaskProtection は対象 task が deploy 時に動的に決まるためリソース ARN を事前に絞れない。kms:Decrypt は SSM SecureString 復号用で default KMS key (alias/aws/ssm) のみに限定済み。X-Ray (PutTraceSegments 等) はリソースレベル制限非対応の API。',
    },
  ]);
};

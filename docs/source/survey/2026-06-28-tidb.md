...
ところが先のクエリ群で参照していた `INFORMATION_SCHEMA.TIKV_REGION_STATUS` は **同じ Region を `(region × インデックスキースペース)` の単位でばらして 1 行ずつ返す**仕様。`articles` には次の 5 つのキースペースがある:

1. 行データ（clustered PK on `article_id`）
2. `uq_articles_slug`
3. `idx_articles_user_id`
4. `idx_articles_status_published_at`
5. （+ 内部メタ）

= **同一物理 Region 1 個 × 5 キースペース = TIKV_REGION_STATUS から見ると 5 行**。これがクエリ 1/2 の `region_count=5` / `peer_cnt=5` の正体。
...
物理 Region 数を正しく知りたい場合は **`SHOW TABLE ... REGIONS` を使う**か、`SELECT COUNT(DISTINCT REGION_ID) ...` で重複を除く必要がある。

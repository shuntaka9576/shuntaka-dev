...
```{figure} 2026-06-28-tidb/region-leader-store.png
...
```{figure} 2026-06-28-tidb/region-leader-store-conceptual.png
...
```{figure} 2026-06-28-tidb/tidb-region-peer-leader-internals.png
...
- [ ] 概念図 `2026-06-28-tidb/region-leader-store.png` を「articles は 1 物理 Region」「将来の自動分割後の予測図は別に併記」の構成に差し替え（別途進行中）

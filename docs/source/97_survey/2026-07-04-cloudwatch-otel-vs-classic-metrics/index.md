...
- 関連:
  ...
  - [運用](../../01_開発ドキュメント/04_operations.md)（ダッシュボード・テレメトリ一覧・ハマりどころ）
- 参考:
...
採用時の比較（[タスクドキュメント](../../98_tasks/2026-07-03-otel-bottleneck-observability/index.md)参照）は OTel 側の利点（コスト ~$10/月 → $1 未満/月、PromQL、dimension 管理不要）のみだったため、逆方向の「Classic だったらできたのにできないこと」を整理する。
...
現状は「リクエスト単位の悉皆データは X-Ray で見る」で回避している（[運用のハマりどころ](../../01_開発ドキュメント/04_operations.md)参照）が、X-Ray にはサンプリングと保持期間の制約があり完全な代替ではない。

...
- 調査日: 2026-06-28
- 関連: [DSQL → TiDB 移行プラン](../../98_tasks/2026-06-26-dsql-to-tidb-migration/index.md)
...
```{figure} region-leader-store.png
...
```{figure} region-leader-store-conceptual.png
...
```{figure} tidb-region-peer-leader-internals.png
...
- [ ] 概念図 `region-leader-store.png` を「articles は 1 物理 Region」「将来の自動分割後の予測図は別に併記」の構成に差し替え（別途進行中）

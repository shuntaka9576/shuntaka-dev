...
TiKV は RocksDB (LSM) ベースの単純な KV ストアで、二次索引は「別のキー空間に張られた通常のレコード」として表現される。B-tree 索引ならこのモデルに乗るが、inverted index は posting list という別種のデータ構造で、更新はセグメントマージ、読み取りはリスト交差と、KV レンジスキャンの語彙に収まらない。TiKV に載せるなら、LSM の上に独立した索引エンジン (Lucene 相当のセグメント管理・マージ・辞書) を作り込み、Raft のトランザクション経路と整合させる必要がある。これは「差し込みにくい」というより新ストレージを一本書くのに近い作業になる。
...
一方 TiFlash は既に Raft Learner としてレプリカを受け取り、自前のストレージエンジン (DeltaTree) を持ち、非同期に構造を作り替えることが許されている。つまり「レプリカに派生構造を作る」ためのフレームが最初から揃っている。inverted index はそこに新しい index 型として追加すればいい。

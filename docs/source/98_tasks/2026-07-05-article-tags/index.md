...
- 起票日: 2026-07-05
- 関連: [記事詳細 API の content_html 事前生成](../2026-07-02-articles-content-html-pregeneration/index.md)
...
本番の論理ダンプを取得して blog_dev に流し込む（ダンプの詳細は [本番 TiDB (blog_prd) の論理ダンプ手順](../2026-07-05-tidb-prd-dump/index.md)）。ダンプはスキーマ名非依存（`CREATE DATABASE` / `USE` を含まない）なので、リストアは `-D blog_dev` を指定するだけでよい。

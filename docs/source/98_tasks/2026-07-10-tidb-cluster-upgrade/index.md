...
- 手元 Mac に `mysql` クライアント（root は現状パスワードなしで接続可。full-rebuild 手順 Phase D のパスワード設定は未実施状態。設定した場合は `-p` を付ける）
...
ローリング中にクライアントから見て接続断が起きていないかを、1 秒間隔の新規接続プローブで観測する。root は現状パスワードなしなのでそのまま回る（パスワードを設定した場合は事前に `export MYSQL_PWD=<password>` してから回す。ループ内 `read` はプロンプトが出ず固まって見えるので使わない）。
...
- 残課題（本作業のスコープ外）: root パスワードが未設定のまま（full-rebuild Phase D 相当）。tailnet 内限定とはいえ設定推奨

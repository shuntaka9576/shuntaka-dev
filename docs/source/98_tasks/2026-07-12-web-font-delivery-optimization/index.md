...
## 効果測定（2026-07-12、PR #610 の Vercel プレビュー環境 HAR）

トップページ 1 ロード分を対応前の本番 HAR と比較。フォント合計で **−221 KiB (−39%)**。

| 項目             | Before (5 ウェイト + CDN CSS) | After (400/600 + self-host CSS) | 削減         |
| ---------------- | ----------------------------- | ------------------------------- | ------------ |
| ウェイト CSS     | 150.5 KiB / 5 本              | 65.3 KiB / 2 本                 | −85 KiB      |
| woff2            | 411.0 KiB / 22 ファイル       | 274.9 KiB / 18 ファイル         | −136 KiB     |
| **フォント合計** | **561.5 KiB**                 | **340.2 KiB**                   | **−221 KiB** |

- 取得 woff2 は 400 ×17 + 600 ×1 のみになり、300 側の重複サブセット（74.6 KiB の大玉含む）と 500/700 の取得が消えたことを確認
- CSS は自ドメイン配信（`/fonts/gen-interface-jp/{400,600}.css`）になり、render-blocking の外部 CDN 依存が解消
- 注: プレビュー環境は GTM / Clarity の env が無いため analytics 分（約 280 KiB）が載っておらず、代わりに vercel.live（52.9 KiB）が入る。ページ総量の直接比較は不可だが、フォント部分は同条件

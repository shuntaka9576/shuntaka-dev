...
- `ts-tidb-public` は node3 に載っており退避不要。`ts-plamo-embedding-public` が node2 に載っていたため、再起動中はセマンティック検索 API が数分止まる（判断基準は [運用 > ノード再起動](../../01_開発ドキュメント/04_operations.md) 参照）

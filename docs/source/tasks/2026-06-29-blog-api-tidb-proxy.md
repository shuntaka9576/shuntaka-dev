...
- 関連調査: [Tailscale + Lambda の ephemeral ノード蓄積](../survey/2026-06-29-tailscale-lambda-ephemeral-pileup.md)
...
- [ ] Tailscale auth key を SSM `/shared/shuntaka/tailscale/proxy-auth-key` に手動で格納（90 日 rotation 運用、手順は `docs/source/01_development.md` を参照）
...
- 調査: [Tailscale + Lambda の ephemeral ノード蓄積](../survey/2026-06-29-tailscale-lambda-ephemeral-pileup.md)
- 前提タスク: [DSQL → TiDB 移行](./2026-06-26-dsql-to-tidb-migration.md)（本タスクは TiDB 移行完了後の改善）

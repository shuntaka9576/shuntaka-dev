...
```{image} ./images/2026-06-25-construction-plan/network.drawio.png
:alt: MiniPC k8s Cluster Network Architecture
:width: 100%
```

AWS Lambda (Rust + tailscaled マルチプロセス) → Tailscale tailnet → Subnet Router(node1) → 管理 VLAN (VLAN 1) → k8s API / TiDB Pod までの経路をまとめてある。

> 注: 図中の "VLAN 10" 表記は初版作成時のもの。実装は SG3210X-M2 の System-VLAN (VLAN 1) をそのまま管理 VLAN として使う方針 (= B 案、後述「SG3210X-M2 VLAN設定」セクション参照) に変更したため、本文の表記とは VID が異なる (図 VLAN 10 = 本文 VLAN 1)。サブネット (`192.168.4.0/22`) と機能は同じ。図は次回再エクスポート時に同期予定。
>
